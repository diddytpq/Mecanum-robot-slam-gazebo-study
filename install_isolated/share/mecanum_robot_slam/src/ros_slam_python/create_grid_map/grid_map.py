...
class GridMap:
    ...
    def numpy_to_occupancy_grid(self, info=None):

        rviz_map = self.l.copy()

        rviz_map[rviz_map > log_odds(TRESHOLD_P_OCC)] = 100
        rviz_map[rviz_map == 0] = 10
        rviz_map[rviz_map < log_odds(TRESHOLD_P_FREE)] = 0
        rviz_map[rviz_map == 10] = -1

        arr = np.int8(rviz_map)

        arr = np.fliplr(arr)

        arr = cv2.rotate(src = arr, rotateCode = cv2.ROTATE_90_COUNTERCLOCKWISE)
        ...
        pos = np.array([-arr.shape[1] * self.resolution / 2, -arr.shape[0] * self.resolution / 2, 0])
        
        quat = np.array([0, 0, 0, 1])
        grid.info.origin = Pose()
        grid.info.origin.position.x, grid.info.origin.position.y = pos[:2]
